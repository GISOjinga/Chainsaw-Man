...
local services: {[string]: Service<any>} = {}
local started = false
...
local function DoesServiceExist(serviceName: string): boolean
	local service: Service<any>? = services[serviceName]
	return service ~= nil
end
...
function KnitServer.CreateService<A>(serviceDef:A):A&Service<A>&ServiceDefe<A>
...
function KnitServer.AddServices<B,A>(parent: Instance): {Service<A>}
	local addedServices = {}
	for _,v in ipairs(parent:GetChildren()) do
		if not v:IsA("ModuleScript") then continue end
		local OOf:B&Service<A> = require(v)
		table.insert(addedServices, OOf)
	end
...
function KnitServer.AddServicesDeep<A>(parent: Instance): {Service<A>}
	local addedServices = {}
...
function KnitServer.GetService<A>(serviceName: string): Service<A>
	assert(started, "Cannot call GetService until Knit has been started")
	assert(type(serviceName) == "string", "ServiceName must be a string; got " .. type(serviceName))
	local meep:A = services[serviceName]
	return assert(meep, "Could not find service \"" .. serviceName .. "\"")::any
end
